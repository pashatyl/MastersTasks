...
def _parse_args():
    ...
    parser.add_argument('--show-failed-examples', action='store_true',
                        help='print file names of examples on which algo has failed')
...
def _run_algo(executable, model, fname):
    cmd = [sys.executable, executable, model, fname]
...
def _main(args):
    ...
    vj = ViolaJonesFramework.load(args.model)
    ...
    for i, category in enumerate(CATEGORIES):
        ...
        for fname in os.listdir(folder):
            ...
            try:
                result = vj.classify(read_file(full_path))
            ...
            if result == i:
                ...
                if True or args.show_failed_examples:
